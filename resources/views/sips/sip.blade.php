    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('sip.sendEmailMassal') }}"
                    class="btn btn-sm btn-outline-success d-flex align-items-center px-3"
                    style="margin-bottom: 0px;line-height: 1; white-space: nowrap;">
                    <i class="fas fa-envelope me-2 fs-6"></i> Kirim Email
                </a>
                <a href="{{ route('sip.sendRekapTelegram') }}"
                    class="btn btn-sm btn-outline-info d-flex align-items-center px-3"
                    style="margin-bottom: 0px;line-height: 1; white-space: nowrap;">
                    <i class="fab fa-telegram me-2 fs-6"></i> Kirim Telegram
                </a>
                <a href="{{ route('sip.sendRekapEmail') }}" class="btn btn-sm btn-success d-flex align-items-center px-3"
                    style="margin-bottom: 0px;line-height: 1; white-space: nowrap;">
                    <i class="fas fa-envelope-open-text me-2 fs-6"></i> Kirim Rekap ke Admin
                </a>
                <a class="btn btn-sm btn-outline-primary d-flex align-items-center px-3" data-bs-toggle="modal"
                    data-bs-target="#schedulerModal" style="margin-bottom: 0px;line-height: 1; white-space: nowrap;">
                    <i class="fas fa-stopwatch fs-6"></i>
                </a>
                <form method="GET" action="{{ route('employees.sip') }}" class="d-flex align-items-center gap-1"
                    style="max-width: 350px; width: 100%;">
                    <input type="text" name="search" class="form-control form-control-sm py-2" style="height: 40px;"
                        placeholder="Cari nama, NIP, SIP, atau email..." value="{{ request('search') }}">
                    <button class="btn btn-sm btn-outline-primary d-flex align-items-center px-2" type="submit"
        ...
        @php
            $config =
                \App\Models\SchedulerSetting::first() ??
                (object) [
                    'day_of_week' => 'sun',
                    'hour' => 9,
                    'minute' => 0,
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="day_of_week" required>
                                @foreach (['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'] as $day)
                                    <option value="{{ $day }}"
                                        {{ $config->day_of_week == $day ? 'selected' : '' }}>
                                        {{ ucfirst($day) }}
                                    </option>
                                @endforeach
                            </select>
        ...
        @if (session('success'))
            <div class="alert alert-success alert-dismissible fade show mt-2" role="alert">
                ...
                <button type="button" class="btn-close" data-bs-dismiss="alert"
                    style="background-image: #ffffff;"></button>
            </div>
        @endif
        ...
        <div class="card shadow-sm">
            <div class="card-body p-3">
                    ...
                    <table class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">No SIP
                                </th>
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">Tgl
                                    Terbit</th>
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">Tgl
                                    Kedaluwarsa</th>
                                <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">Sisa
                                    Hari</th>
                        ...
                        <tbody>
                            @foreach ($employees as $index => $employee)
                                <tr>
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">{{ $employee->no_sip ?? '-' }}</p>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">
                                            @php
                                                $sisaHari = \Carbon\Carbon::parse(
                                                    $employee->tanggal_kadaluwarsa,
                ...
                <div class="d-flex justify-content-end mt-3">
                    {{ $employees->appends(['search' => request('search')])->links('pagination::bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
